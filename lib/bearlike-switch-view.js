...
export class BearlikeSwitchView {
  ...
  activate() {
    ...
    inkdrop.onEditorLoad((_) => {
      const isVisible = inkdrop.config.get("core.mainWindow.sideBar.visible");
  ...
  toggle_column_one() {
    ...
    this.query(".editor-header-top-spacer").style.height = "16px";
    this.query(".editor-meta-layout").style.display = "none";
  }
  /*
   *
   */
  toggle_column_two() {
    this.query(".editor-header-top-spacer").style.height = "0px";
    this.query(".editor-meta-layout").style.display = "";
    // sidebar が出ている場合は非表示
  ...
  isSideBarVisibleState() {
    const value = inkdrop.config.get("core.lastNavigationState.sidebar.workspace.visible")
    if (value != undefined) {
      return value
    }
